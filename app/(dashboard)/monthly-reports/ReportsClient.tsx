    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
      ...
      <div className="lg:col-span-1 select-none">
        <div className="glass p-4 sm:p-5 space-y-3">
          <div>
            <h2 className="text-sm sm:text-base font-bold">Monthly Summaries</h2>
            <p className="text-xs text-muted-foreground mt-0.5">Select a month to view the detailed summary.</p>
          </div>
      ...
      {/* View report */}
      <div className="lg:col-span-2 select-none print:hidden h-full">
        {!selectedReport ? (
          <div className="glass p-12 text-center text-muted-foreground flex flex-col items-center gap-2 justify-center h-full min-h-[300px]">
            ...
            <p className="text-xs">Pick a monthly summary to view or download.</p>
          </div>
        ) : (
          <div className="glass p-4 sm:p-6 space-y-6 flex flex-col justify-between h-full">
            <div className="flex items-center justify-between gap-4 flex-wrap pb-4 border-b">
              <div>
                ...
                <p className="text-xs text-muted-foreground mt-0.5">Financial summary and reconciliation.</p>
              </div>
              <button
                onClick={handlePrint}
            ...
            <div className="space-y-4 border-t pt-4">
              <h3 className="text-sm sm:text-base font-bold">Additional Breakdowns</h3>

              {/* Outstanding Dues list */}
              <div className="space-y-2">
                <div className="text-xs font-semibold text-muted-foreground">Outstanding dues for that month</div>
                      ...
                      {selectedReport.details.dueList.map((d) => (
                        <tr key={d.userId} className="hover:bg-muted/30">
                          ...
                          <td className={`px-3 py-2 font-bold text-right ${d.due > 0 ? "text-[var(--red)]" : "text-[var(--green)]"}`}>
                            {d.due > 0 ? `৳${d.due.toLocaleString()}` : "✓ Paid"}
                          </td>
                        </tr>
                      ))}
                    </tbody>
              ...
              {/* Deposits */}
              <div className="space-y-2">
                <div className="text-xs font-semibold text-muted-foreground">Month's Deposits</div>
                <div className="overflow-x-auto border rounded-lg max-h-[200px] overflow-y-auto">
                  <table className="w-full text-xs text-left">
                    <thead className="bg-muted/50 sticky top-0 backdrop-blur">
                      <tr>
